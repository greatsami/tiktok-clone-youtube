...
<template>
  ...
  <div v-if="isUploading" class="fixed flex items-center justify-center top-0 left-0 w-full h-screen bg-black z-50 bg-opacity-50">
    <Icon name="mingcute:loading-line" size="100" color="#ffffff" class="animate-spin ml-1" />
  </div>
    ...
    <div class="w-full mt-[80px] mb-[40px] bg-white shadow-lg rounded-md py-6 md:px-10 px-4">
      ...
      <div class="mt-8 md:flex gap-6">
        <label
            ...
            @drop.prevent="onDrop"
            @dragover.prevent=""
            for="fileInput" class="md:mx-0 mx-auto mt-4 mb-6 flex flex-col items-center justify-center w-full max-w-[260px] h-[470px] text-center p-3 border-2 border-dashed border-gray-300 rounded-lg hover:bg-gray-100 cursor-pointer">
          <Icon name="majesticons:cloud-upload" size="40" color="#b3b3b1" />
          <span class="mt-4 text-[17px]">Select video to upload</span>
          <span class="mt-1.5 text-gray-500 text-[13px]">Or drag and drop a file</span>
          <span class="mt-12 text-gray-400 text-sm">MP4</span>
          <span class="mt-2 text-gray-400 text-[13px]">Up to 30 minutes</span>
          <span class="mt-2 text-gray-400 text-[13px]">Less than 2 GB</span>
          <span class="px-2 py-1.5 mt-8 text-white text-[15px] w-[80%] bg-[#f02c56] rounded-sm">
            Select file
          </span>
        ...
        <div v-else class="md:mx-0 mx-auto mt-4 md:mb-12 mb-16 flex items-center justify-center w-full max-w-[260px] h-[540px] p-3 rounded-xl cursor-pointer relative">
          <div class="bg-black h-full w-full" />

          <img src="~/assets/images/mobile-case.png" class="absolute z-20 pointer-events-none" alt="" />
          <img src="~/assets/images/tiktok-logo-white.png" class="absolute right-4 bottom-6 z-20" width="90" alt="" />
        ...
        <div class="mt-4 mb-6">
          <div class="flex bg-[#f8f8f8] py-4 px-6">
            ...
            <div>
              <div class="font-semibold text-[15px] mb-1.5">Divide videos and edit</div>
          ...
          <div class="mt-5">
            ...
            <input type="text" v-model="caption" class="w-full border p-2.5 rounded-md focus:outline-none" maxlength="150" />
          </div>

          <div class="flex gap-3">
            <button @click="$event => discard()" class="px-10 py-2.5 mt-8 border text-[16px] hover:bg-gray-100 rounded-sm">
              Discard
            </button>
            <button
          ...
          <div v-if="errors" class="mt-4">
            <div class="text-red-600" v-if="errors && errors.video">{{ errors.video[0] }}</div>
            <div class="text-red-600" v-if="errors && errors.text">{{ errors.text[0] }}</div>
          </div>
        </div>
